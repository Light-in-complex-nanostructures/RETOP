...
%  
%  b=a(k,:)  a 'voisin' de b(kk,:)
% marche aussi pour des cell array
% EXEMPLE
...
n0=n0+1;
k(n0)=ii;
if iscell(a);f=ii;for jj=ii+1:n;
        if abs(retcompare(size(a{ii}),size(a{jj})))<1.e-6;% ajout 5 2015
        if abs(retcompare(a{ii},a{jj}))<tol;f=[jj;f(:)];end;
        end;
    end;
